...
- `supabase/functions/graph-api/` — the backend (Supabase Edge Function). Research/news use OpenRouter (`openai/gpt-5-nano` + one grounded web search per call, not an agentic search loop); Apollo's free `organizations/enrich` backfills whatever OpenRouter didn't find.
- `schema.sql`, `migration_002_sectors_and_news.sql` — run once each in the Supabase SQL Editor, in order.
...
`OPENROUTER_API_KEY`, `APOLLO_API_KEY`, `ALLOWED_EMAIL` — set via:
